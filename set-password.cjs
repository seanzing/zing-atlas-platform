 ...
 * Usage: node set-password.cjs <email> <password>
 */
...
if (!email || !password) {
  console.error("Usage: node set-password.cjs <email> <password>");
  process.exit(1);
}
...
async function run() {
  ...
  console.log(`✅ Password set for ${email} — they can log in at https://zing-atlas-platform-production.up.railway.app/login`);
}
